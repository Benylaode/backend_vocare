...
def create_laporan():
    ...
    if os.path.exists(FAISS_INDEX_FILE) and os.path.exists(MAPPING_FILE):
        try:
            with open(MAPPING_FILE, "rb") as f: id_to_text = pickle.load(f)
            index = faiss.read_index(FAISS_INDEX_FILE)
            matches = search_with_faiss(query, index, id_to_text, k=5)
    ...
    try:
        completion = client.chat.completions.create(
            model=api_model,
            messages=[
                {"role": "system", "content": "Anda adalah perawat profesional. Susun laporan ASKEB (Asuhan Keperawatan). Output JSON valid: {subjective, objective, plan, tindakan_lanjutan, keterangan, SDKI:[], SIKI:[], SLKI:[]}."},
                {"role": "user", "content": f"Kasus Pasien: {query}\nReferensi: {context_text}"}
            ],
        )
        ...
        parsed = {"subjective": query, "objective": "-", "plan": "-", "tindakan_lanjutan": "-", "SDKI": [], "SIKI": [], "SLKI": []}
    ...
    return jsonify({"status": 201, "message": "Laporan (Askeb) berhasil dibuat", "data": laporan_to_dict(laporan)}), 201
...
def laporan_to_dict(l):
    return {
        "id": l.id, "patient_id": l.patient_id, "tanggal": l.tanggal.isoformat(),
        "subjective": l.subjective, "objective": l.objective, "assessment": l.assessment,
        "plan": l.plan, "SDKI": l.SDKI, "SIKI": l.SIKI, "SLKI": l.SLKI
    }
